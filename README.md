...
**折り方が違えば、ページの並びも変わります。** 同じ 8 ページの折本でも、
キンコーズの図6 と海外でよくある zine 折りでは、表紙の来るマスが 2 つずれます。

自分の並びは、**折ったメモにめくる順で 1〜8 と書いてから開けば**分かります。
それをそのまま指定できます。

```bash
# 上の段から順に、開いて置いたときに見えるとおりに書く
orihon unimpose スキャン.pdf --layout "7,6,5,4/8,1,2,3"   # 図6（既定）
orihon unimpose スキャン.pdf --layout "5,4,3,2/6,7,8,1"   # よくある zine 折り
orihon unimpose スキャン.pdf --layout "8,7/1,6/2,5/3,4"   # 天綴じ（2列4段）
```

回転角は折り線の向きから自動で決まるので、書く必要はありません。
**折本として成立しない並びは受け付けません**（連続するページが
隣り合っていない、など）。`orihon impose` でも同じ指定が使えます。
